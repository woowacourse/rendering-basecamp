...
export const usePopularMovies = (initialMovies: MovieItem[] | undefined) => {
  const [data, setData] = useState<MovieItem[] | null>(initialMovies ?? null);
  const [isLoading, setIsLoading] = useState(initialMovies == null || initialMovies.length === 0);
  const [error, setError] = useState<Error | null>(null);

  useEffect(() => {
    if (initialMovies != null && initialMovies.length > 0) {
      return;
    }
